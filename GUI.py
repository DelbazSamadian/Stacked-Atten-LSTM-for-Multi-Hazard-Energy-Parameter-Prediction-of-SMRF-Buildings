...
APP_TITLE = "Total Energy Prediction App for SMRF Buildings Exposed to Sequential Earthquake–Flood Hazards"
APP_SUBTITLE = "Hybrid Static-Feature + Attention-LSTM Surrogate Model"
...
st.set_page_config(
    page_title="Total Energy Prediction App for Multi-Hazard Response Prediction of SMRF Buildings",
    page_icon="🏗️",
    layout="wide",
    initial_sidebar_state="expanded",
)

# Same Teesside University logo source used in the previous MIDR GUI.
TEESSIDE_LOGO_URL = (
    "https://github.com/DelbazSamadian/"
    "Stacked-Atten-LSTM-for-Multi-Hazard-Response-Prediction-of-SMRF-Buildings/"
    "blob/main/OIP.jpeg?raw=true"
)
...
st.markdown(
    ...
    .stMarkdown, .stSubheader, .stHeading, .stSuccess,
    .stWarning, .stInfo, .stError, .stCaption {
        font-family: 'Times New Roman', Times, serif;
    }
...
# Header: logo, university, developers, description, and usage instructions
# Keep the HTML compact (without Markdown-indented blocks) so Streamlit renders
# it as HTML rather than displaying the tags as a code block.
header_html = (
    f'<div class="app-header">'
    f'<img src="{TEESSIDE_LOGO_URL}" alt="Teesside University Logo">'
    f'<div>'
    f'<h1>Total Energy Prediction App for SMRF Buildings Exposed to '
    f'Sequential Earthquake–Flood Hazards</h1>'
    f'<p>Developed by <strong>Teesside University</strong></p>'
    f'<p><strong>Developers:</strong> Delbaz Samadian, Annalisa Occhipinti, '
    f'Imrose B. Muhit, Nashwan Dawood</p>'
    f'<p style="margin-top:10px;">'
    f'This open-access research app predicts the Total Energy response of '
    f'steel special moment-resisting frame (SMRF) buildings exposed to '
    f'sequential earthquake and flood scenarios using the trained hybrid '
    f'Attention-LSTM surrogate model.'
    f'</p>'
    f'<div style="margin-top:10px;">'
    f'<h3>How the App Works</h3>'
    f'<ul>'
    f'<li>Select the earthquake intensity and flood height.</li>'
    f'<li>Set the ground-motion time step (Δt) when required and upload '
    f'acceleration records in the X and Y directions.</li>'
    f'<li>Specify the building using a reference building, the quick custom '
    f'mode, or the full structural-input CSV mode.</li>'
    f'<li>Click <strong>Predict Total Energy</strong> to estimate the '
    f'multi-hazard energy response.</li>'
    f'</ul>'
    f'</div>'
    f'</div>'
    f'</div>'
...
def trapezoidal_integral(y: np.ndarray, x: np.ndarray) -> float:
    ...
    np.trapz was deprecated in NumPy 2.0 and removed in later releases.
    np.trapezoid is the replacement in modern NumPy. This helper also keeps
...
def summarise_channel(time_values, signal_values):
    ...
    if signal_values.size >= 2 and duration > 0:
        squared_integral = float(
            trapezoidal_integral(np.square(signal_values), time_values)
        )
    else:
